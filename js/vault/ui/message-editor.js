             ...
             <div class="field">
                 ...
                 <div class="copy-container">
                     ...
                     <div class="icon-actions">
                         ...
                         <button class="icon-btn favorite-btn" id="toggleFavoriteBtn" title="${msg.isImportant ? 'Favorilerden Çıkar' : 'Favorilere Ekle'}">
                            ${msg.isImportant
                    ? '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="#ff9500"><path fill-rule="evenodd" d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.006 5.404.434c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.434 2.082-5.005Z" clip-rule="evenodd" /></svg>'
        ...
        if (isNew) {
            ...
            if (favoriteBtn) {
                favoriteBtn.onclick = async () => {
                    ...
                    if (success) {
                        showCustomToast(msg.isImportant ? 'Favorilere eklendi!' : 'Favorilerden çıkarıldı!');
                        // UI'ı güncelle
                        updateEditorUI();
                    } else {
                        // Hata durumunda geri al
                        msg.isImportant = !msg.isImportant;
                        showCustomToast('Bir hata oluştu. Lütfen tekrar deneyin.');
                    }
                };
            }
        }
...
export async function handleSaveMessage(password, messages, index) {
    ...
    if (index === -1) {
        ...
        // Umami Analytics: Not oluşturma
        if (typeof umami !== 'undefined' && typeof umami.track === 'function') {
            umami.track('note_created', { notes_count: messages.length + 1 });
        }
    } else {
        messages[index] = newMessage;
        // Umami Analytics: Not düzenleme
        if (typeof umami !== 'undefined' && typeof umami.track === 'function') {
            umami.track('note_edited');
        }
    }
...
export async function handleDeleteMessage(password, messages, index) {
    ...
    if (msg.isDeleted || !trashEnabled) {
        ...
        if (confirmed) {
            ...
            if (typeof umami !== 'undefined' && typeof umami.track === 'function') {
                umami.track('note_permanently_deleted');
            }
        ...
        if (confirmed) {
            ...
            if (typeof umami !== 'undefined' && typeof umami.track === 'function') {
                umami.track('note_moved_to_trash');
            }
